...
out_dir <- file.path(root, "outputs", "nature_r_figures_v3")
...
source_exit <- high$exit_group %>%
  ...
  mutate(exit_short = factor(exit_short, levels = top_exits$exit_short),
         line = factor(line, levels = c("L16", "L18", "L2", "L7", "Maglev"))) %>%
  complete(line, exit_short, fill = list(people = 0))

p3a <- ggplot(source_exit, aes(exit_short, line, fill = people)) +
  geom_tile(colour = "white", linewidth = 0.3) +
  scale_fill_gradient(low = "#F1EFE6", high = pal$adaptive, name = "Passengers") +
  labs(title = "Adaptive source-to-exit allocation", x = NULL, y = NULL) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right", axis.line = element_blank(), axis.ticks = element_blank())
...
fig3 <- p3a / (p3b | p3c) + plot_layout(heights = c(1.05, 1), widths = c(1, 1)) + panel_tag()
...
p4b <- ggplot(top_fac, aes(people, facility_short, fill = method)) +
  geom_col(position = position_dodge(width = 0.62), width = 0.54) +
  scale_fill_manual(values = method_cols) +
  labs(title = "High-throughput facilities", x = "Passengers", y = NULL) +
...
fig4 <- (p4a | p4b) / p4c + plot_layout(heights = c(1, 0.72), widths = c(1, 1)) + panel_tag()
save_pub(fig4, "fig4_facility_mechanism", 183, 128)
...
readme <- c(
  ...
  "3. fig3_flow_redistribution: source-to-exit allocation, exit share shifts, and line clearance redistribution.",
  "4. fig4_facility_mechanism: facility load shifts, high-throughput facilities, and route-chain burden.",
