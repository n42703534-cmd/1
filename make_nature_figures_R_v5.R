...
out_dir <- file.path(root, "outputs", "nature_r_figures_v5")
...
# Fig. 3-5 v5 overwrite: clearer narrative panels ----------------------------
source_exit_v5 <- high$exit_group %>%
  mutate(exit_short = gsub("Exit_", "", exit_name)) %>%
  group_by(method_label, line, exit_short) %>%
  summarise(people = sum(people), .groups = "drop")

top_exits_v5 <- source_exit_v5 %>%
  filter(method_label == adaptive_key) %>%
  group_by(exit_short) %>%
  summarise(total = sum(people), .groups = "drop") %>%
  slice_max(total, n = 9) %>%
...
route_bubbles_v5 <- source_exit_v5 %>%
  filter(method_label == adaptive_key, exit_short %in% top_exits_v5$exit_short) %>%
  group_by(line) %>%
  mutate(line_total = sum(people), share = people / line_total) %>%
  ungroup() %>%
  filter(people > 25) %>%
  mutate(
    line = factor(line, levels = rev(c("L16", "L18", "L2", "L7", "Maglev"))),
    exit_short = factor(exit_short, levels = rev(top_exits_v5$exit_short)),
    label = if_else(people >= 450, comma(round(people)), "")
  )

p3a_v5 <- ggplot(route_bubbles_v5, aes(exit_short, line)) +
  geom_point(aes(size = people, fill = line), shape = 21, colour = "white", stroke = 0.35, alpha = 0.9) +
  geom_text(aes(label = label), size = 2.05, colour = "white", fontface = "bold") +
  scale_fill_manual(values = line_cols, guide = "none") +
  scale_size_area(max_size = 13, breaks = c(250, 750, 1250), labels = comma, name = "Passengers") +
  labs(
    title = "Adaptive routing assigns major source lines to a small set of exits",
    subtitle = "High-load demand; circle area encodes passengers assigned from a line to an exit",
    x = "Exit used by adaptive routing",
    y = "Source line"
  ) +
  theme(
    ...
    panel.grid.major.x = element_line(linewidth = 0.25, colour = pal$light),
    panel.grid.major.y = element_line(linewidth = 0.25, colour = pal$light)
  )

exit_delta_v5 <- high$exits %>%
  mutate(delta_pp = .data[[paste0(adaptive_key, "_pct")]] - .data[[paste0(baseline_key, "_pct")]]) %>%
  slice_max(abs(delta_pp), n = 10) %>%
  arrange(delta_pp) %>%
  mutate(
    exit_short = gsub("Exit_", "", exit),
    exit_short = factor(exit_short, levels = exit_short),
    direction = if_else(delta_pp >= 0, "More use", "Less use"),
    lab = sprintf("%+.1f", delta_pp)
  )

p3b_v5 <- ggplot(exit_delta_v5, aes(delta_pp, exit_short)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = pal$neutral) +
  geom_col(aes(fill = direction), width = 0.55) +
  geom_text(aes(label = lab, hjust = if_else(delta_pp > 0, -0.15, 1.15)), size = 2.15) +
  scale_fill_manual(values = c("More use" = pal$adaptive, "Less use" = pal$baseline), guide = "none") +
  scale_x_continuous(expand = expansion(mult = c(0.12, 0.16))) +
  labs(title = "Exit shares are deliberately redistributed", x = "Adaptive - baseline share (percentage points)", y = NULL) +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank())

line_df_v5 <- bind_rows(low$line, high$line) %>%
  select(load, line, all_of(c(baseline_key, adaptive_key))) %>%
  rename(Baseline = all_of(baseline_key), Adaptive = all_of(adaptive_key)) %>%
  pivot_longer(c(Baseline, Adaptive), names_to = "method", values_to = "clearance") %>%
  mutate(
    line_load = paste(load, line),
    line_load = factor(line_load, levels = rev(paste(rep(c("Low-load", "High-load"), each = 5), rep(c("L16", "L18", "L2", "L7", "Maglev"), 2)))),
    method = factor(method, levels = c("Baseline", "Adaptive"))
  )

p3c_v5 <- ggplot(line_df_v5, aes(clearance, line_load, colour = method)) +
  geom_line(aes(group = line_load), colour = pal$light, linewidth = 0.7) +
  geom_point(size = 2.2) +
  scale_colour_manual(values = method_cols, name = NULL) +
  labs(title = "Redistribution changes clearance by line", x = "Line clearance time (s)", y = NULL) +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(), legend.position = "top")

fig3_v5 <- p3a_v5 / (p3b_v5 | p3c_v5) + plot_layout(heights = c(1.25, 0.88), widths = c(1, 1.04)) + panel_tag()
save_pub(fig3_v5, "fig3_flow_redistribution", 183, 128)

short_node_v5 <- function(x) {
  x <- gsub("^VN_", "", x)
  x <- gsub("^Gate_", "Gate ", x)
  x <- gsub("^Stair_", "Stair ", x)
  x <- gsub("^Escalator_", "Esc. ", x)
  x <- gsub("^Platform_", "Platform ", x)
  x <- gsub("_Entrance$", " entrance", x)
  x <- gsub("_Arrival$", " arrival", x)
  x <- gsub("_Corner_", " corner ", x)
  x <- gsub("_", " ", x)
  ifelse(nchar(x) > 24, paste0(substr(x, 1, 22), "..."), x)
}

facility_v5 <- high$facility %>%
  mutate(
    delta = .data[[adaptive_key]] - .data[[baseline_key]],
    abs_delta = abs(delta),
    facility_short = short_node_v5(facility),
    type_label = recode(type,
      gate_wide = "Gate",
      stair = "Stair",
      escalator = "Escalator",
      exit = "Exit",
      virtual = "Transfer node",
      platform_waiting_zone = "Platform zone",
      .default = "Other"
    ),
    direction = if_else(delta >= 0, "More passengers", "Fewer passengers")
  )

shift_fac_v5 <- facility_v5 %>%
  slice_max(abs_delta, n = 12) %>%
  arrange(delta) %>%
  mutate(facility_short = factor(facility_short, levels = facility_short))

p4a_v5 <- ggplot(shift_fac_v5, aes(delta, facility_short)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = pal$neutral) +
  geom_col(aes(fill = direction), width = 0.56) +
  geom_text(aes(label = sprintf("%+d", round(delta)), hjust = if_else(delta > 0, -0.1, 1.1)), size = 2.1) +
  scale_fill_manual(values = c("More passengers" = pal$adaptive, "Fewer passengers" = pal$baseline), guide = "none") +
  scale_x_continuous(expand = expansion(mult = c(0.18, 0.2))) +
  labs(
    title = "Adaptive routing unloads bottlenecks and uses spare gate capacity",
    subtitle = "Largest high-load facility changes; labels are passenger-count differences",
    x = "Adaptive - baseline passengers",
    y = NULL
  ) +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank())

type_delta_v5 <- facility_v5 %>%
  group_by(type_label) %>%
  summarise(delta = sum(delta), .groups = "drop") %>%
  filter(abs(delta) > 150) %>%
  arrange(delta) %>%
  mutate(
...
p4b_v5 <- ggplot(type_delta_v5, aes(delta / 1000, type_label)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = pal$neutral) +
  geom_col(aes(fill = direction), width = 0.52) +
  geom_text(aes(label = sprintf("%+.1f", delta / 1000), hjust = if_else(delta > 0, -0.12, 1.12)), size = 2.15) +
  scale_fill_manual(values = c("More passengers" = pal$adaptive, "Fewer passengers" = pal$baseline), guide = "none") +
  scale_x_continuous(expand = expansion(mult = c(0.18, 0.2))) +
  labs(title = "Net load shift by facility class", x = "Passenger difference (10^3)", y = NULL) +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank())

route_layer_v5 <- high$route %>%
  group_by(method, chain_type) %>%
  summarise(people = sum(people), .groups = "drop") %>%
  mutate(
    method = recode(method, !!baseline_key := "Baseline", !!adaptive_key := "Adaptive"),
    chain_type = recode(chain_type, facility = "Facility passages", exit = "Exit passages"),
    chain_type = factor(chain_type, levels = c("Exit passages", "Facility passages"))
  )

p4c_v5 <- ggplot(route_layer_v5, aes(people / 1000, chain_type, colour = method)) +
  geom_line(aes(group = chain_type), colour = pal$light, linewidth = 0.9) +
  geom_point(size = 2.6) +
  scale_colour_manual(values = method_cols, name = NULL) +
  labs(title = "Route-chain burden remains controlled", x = "Passenger appearances (10^3)", y = NULL) +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(), legend.position = "top")

fig4_v5 <- p4a_v5 / (p4b_v5 | p4c_v5) + plot_layout(heights = c(1.08, 0.82), widths = c(1, 1)) + panel_tag()
save_pub(fig4_v5, "fig4_facility_mechanism", 183, 116)

abl_effects_v5 <- abl %>%
  filter(variant %in% c("ImprovedAStar", "Full model", "NoWaitingTime (Density)")) %>%
  select(variant, T100, queue, congestion, severe, R_area, gini) %>%
  pivot_longer(-variant, names_to = "metric", values_to = "value") %>%
  group_by(metric) %>%
  mutate(
    baseline = value[variant == "ImprovedAStar"],
    change_pct = (value - baseline) / baseline * 100
  ) %>%
  ungroup() %>%
  filter(variant != "ImprovedAStar") %>%
  mutate(
    variant = recode(variant, `Full model` = "Full adaptive", `NoWaitingTime (Density)` = "Without queue-density"),
    metric = recode(metric,
      T100 = "Full clearance time",
      queue = "Queueing burden",
      congestion = "Congestion exposure",
      severe = "Severe congestion",
      R_area = "Risk area",
      gini = "Exit imbalance"
    ),
    metric = factor(metric, levels = rev(c("Full clearance time", "Queueing burden", "Congestion exposure", "Severe congestion", "Risk area", "Exit imbalance"))),
    variant = factor(variant, levels = c("Full adaptive", "Without queue-density"))
  )

label_points_v5 <- abl_effects_v5 %>%
  filter(metric %in% c("Queueing burden", "Congestion exposure", "Severe congestion", "Full clearance time")) %>%
  mutate(
    lab = sprintf("%+.0f%%", change_pct),
    hjust = if_else(change_pct >= 0, -0.12, 1.12)
  )

p5_v5 <- ggplot(abl_effects_v5, aes(change_pct, metric, colour = variant)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = pal$neutral) +
  geom_line(aes(group = metric), colour = pal$light, linewidth = 0.9) +
  geom_point(size = 3.2) +
  geom_text(data = label_points_v5, aes(label = lab, hjust = hjust), size = 2.35, show.legend = FALSE) +
  annotate("text", x = -42, y = 5.55, label = "Lower is better", size = 2.25, colour = pal$adaptive, hjust = 0) +
  annotate("segment", x = -6, xend = -34, y = 5.55, yend = 5.55, arrow = arrow(length = unit(1.8, "mm")), colour = pal$adaptive, linewidth = 0.35) +
  annotate("text", x = 82, y = 5.55, label = "Safety penalty after removal", size = 2.25, colour = pal$risk, hjust = 1) +
  annotate("segment", x = 8, xend = 74, y = 5.55, yend = 5.55, arrow = arrow(length = unit(1.8, "mm")), colour = pal$risk, linewidth = 0.35) +
  scale_colour_manual(values = c("Full adaptive" = pal$adaptive, "Without queue-density" = pal$warning), name = NULL) +
  scale_x_continuous(limits = c(-52, 106), breaks = c(-50, -25, 0, 25, 50, 75, 100), labels = function(x) paste0(x, "%")) +
  labs(
    title = "Removing queue-density control reverses the safety gain",
    subtitle = "Percentage change relative to the baseline algorithm under high-load demand",
    x = "Change relative to baseline",
    y = NULL
  ) +
  theme(
    legend.position = "top",
...
save_pub(p5_v5, "fig5_ablation_evidence", 150, 82)

# Final polish overwrite for Fig. 4 and Fig. 5 --------------------------------
fig4_v5b <- p4a_v5 / p4b_v5 + plot_layout(heights = c(1.08, 0.78)) + panel_tag()
save_pub(fig4_v5b, "fig4_facility_mechanism", 160, 108)

label_points_v5b <- abl_effects_v5 %>%
  filter(metric %in% c("Queueing burden", "Congestion exposure", "Severe congestion")) %>%
  mutate(
    lab = sprintf("%+.0f%%", change_pct),
    hjust = if_else(change_pct >= 0, -0.12, 1.12)
  )

p5_v5b <- ggplot(abl_effects_v5, aes(change_pct, metric, colour = variant)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = pal$neutral) +
  annotate("rect", xmin = -52, xmax = 0, ymin = -Inf, ymax = Inf, fill = pal$adaptive, alpha = 0.035) +
  annotate("rect", xmin = 0, xmax = 106, ymin = -Inf, ymax = Inf, fill = pal$warning, alpha = 0.035) +
  geom_line(aes(group = metric), colour = pal$light, linewidth = 0.9) +
  geom_point(size = 3.2) +
  geom_text(data = label_points_v5b, aes(label = lab, hjust = hjust), size = 2.45, show.legend = FALSE) +
  annotate("text", x = -49, y = 6.35, label = "Improvement", size = 2.35, colour = pal$adaptive, hjust = 0) +
  annotate("text", x = 103, y = 6.35, label = "Deterioration", size = 2.35, colour = pal$warning, hjust = 1) +
  scale_colour_manual(values = c("Full adaptive" = pal$adaptive, "Without queue-density" = pal$warning), name = NULL) +
  scale_x_continuous(limits = c(-52, 106), breaks = c(-50, -25, 0, 25, 50, 75, 100), labels = function(x) paste0(x, "%")) +
  labs(
    title = "Queue-density control is required to keep congestion risk low",
    subtitle = "Percentage change relative to the baseline algorithm under high-load demand",
    x = "Change relative to baseline",
    y = NULL
  ) +
...
save_pub(p5_v5b, "fig5_ablation_evidence", 150, 82)

readme <- c(
  "# R Nature-style figure set v5",
  "",
  "Backend: R only (ggplot2 + patchwork + svglite + ragg).",
  "Scenario labels: mode1 -> Low-load; mode4 -> High-load.",
  "",
  "Design reset after inspecting the supplied paper and high-impact evacuation/routing figure patterns:",
  ...
  "3. fig3_flow_redistribution: source-to-exit allocation as a readable bubble map, plus exit-share and line-clearance changes.",
  "4. fig4_facility_mechanism: bottleneck load transfer by facility and facility class, plus route-chain burden.",
  "5. fig5_ablation_evidence: single clean ablation-effect plot; the previous matrix/card collage is removed.",
  ""
)
